...
def get_val_gen(params, labels = False, target_data = None):
	if target_data:  # if validating on non-training species
		index = params.target_species.index(target_data)
		return iu.get_generator(params.targetvalfile[index], params.valbatchsize[index], labels)
...
# no longer used
def val(params, model, target_data = None, save = False):
	...
	if target_data:
		index = params.target_species.index(target_data)
		steps = params.target_val_steps[index]
