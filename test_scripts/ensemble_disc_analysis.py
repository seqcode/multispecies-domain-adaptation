...
RESULT_ROOT = ROOT + "model_out/ensemble_discriminative_analysis/"
...
for tf in TFS:
	...
	ensemble_preds = get_preds_file(tf, "all-but-"+test_species, test_species, model_type)
	ensemble_preds = np.load(ensemble_preds)
	...
	for i in range(min(len(singlespecies_preds), len(ensemble_preds))):
		line = f.readline()
		if line.split()[-1] == "1":
			if(ensemble_preds[i] > 0.5 and singlespecies_preds[i] > 0.5):
				f_matched.write(line)
			elif(ensemble_preds[i] < 0.5 and singlespecies_preds[i] > 0.5):
