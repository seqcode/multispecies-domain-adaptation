...
def load_keras_model(model_file, n, DA = False, BA = False):
    ...
    if DA:
        ...
        return keras.models.load_model(model_file,
                custom_objects = {"GradientReversal":GradientReversal,
                                  ...
                                  "custom_loss_class":custom_loss_class_wrapper(n)})
    if BA:
        # need to tell Keras how the GRL and the custom loss was implemented
        # (these need to match the definitions from when the model was saved)
        return keras.models.load_model(model_file,
...
def get_models_all_runs(tf, train_species, model_type, runs = 1):
    ...
    for run in range(runs):
        ...
        if model_type == "DA_model":
            models.append(load_keras_model(model_file, n, DA = True))
        elif model_type == "BA_model":
            models.append(load_keras_model(model_file, n, BA = True))
        else:
    ...
    for tf in TFS:
        for model_type in model_types:
            ...
            if model_type == "basic_model":
                trained = train_species
            else:
                trained = "all-but-" + train_species

            if model_type == "ensemble_model_reg":

                model_file_prefix = ROOT + "/".join(["models", tf, train_species + "_trained", model_type]) + "/" 
                files = [f for f in os.listdir(model_file_prefix)]
                model_file = max([model_file_prefix + f for f in files], key=os.path.getctime)

                regr = joblib.load(model_file)

                DATA_ROOT = ROOT + "model_out/"
                if tf == "CEBPA":
                    specs = [spec for spec in SPECIES if spec != train_species]
                    files = [DATA_ROOT + tf+"_basic_model_"+species+"-trained_" + train_species + "-test.preds.npy" for species in specs]
                else:
                    specs = [spec for spec in SPECIES_SMALL if spec != train_species]
                    files = [DATA_ROOT + tf+"_basic_model_"+species+"-trained_" + train_species + "-test.preds.npy" for species in specs]
                
                inp = np.load(files[0])
                for f in files[1:]:
                    inp = np.concatenate((inp, np.load(f)), axis=1)

                preds = regr.predict(inp)

                preds_file = get_preds_file(tf, trained, train_species, model_type)
                np.save(preds_file, np.expand_dims(preds, axis=1))

            else:

                
                for test_species in SPECIES:
                    print("\n===== " + tf + " " + test_species + " test, " + trained + " trained " + model_type + "  =====\n")
